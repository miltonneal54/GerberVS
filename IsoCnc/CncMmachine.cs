...
namespace Cnc
{
    public class Machine
    {
        ...
        static string drill_code = "G82 X{x:0.0000} Y{y:0.0000} Z{depth:0.0000} F{feed_rate:0.00} R{lift_z:0.0000} P{pause:0.00}";
        ...
        double drill_depth_z = 0;
        double drill_lift_z = 2;
        double drill_feed_rate = 100;
        double drill_pause_z = 0;
        string drill_format;
        ...
        public Machine(TextWriter tw, ConfigDictionary config)
        {
            ...
            if (config != null ) 
            {
                config.string_get_value("rapid_move_code", ref rapid_move_code);
                config.string_get_value("linear_move_code", ref linear_move_code);
                config.string_get_value("drill_code", ref drill_code);
        ...
        public void drillMove(double x_abs, double y_abs, double? z_depth = null, double? z_lift = null, double? feed_rate = null, double? pause = null)
        {
            ...
            drill_depth_z = z_depth ?? drill_depth_z;
            drill_lift_z = z_lift ?? drill_lift_z;
            drill_feed_rate = feed_rate ?? drill_feed_rate;
            drill_pause_z = pause ?? drill_pause_z;

            if (generate_relative_code)
                tw.WriteLine(drill_format, x_abs - x_coord, y_abs - y_coord, drill_depth_z, drill_lift_z, drill_feed_rate, drill_pause_z);
            else
                tw.WriteLine(drill_format, x_abs, y_abs, drill_depth_z, drill_lift_z, drill_feed_rate, drill_pause_z);
            x_coord = x_abs;
            y_coord = y_abs;
        }
